...
@pytest.mark.asyncio
async def test_reponer_draft_jugador_no_encontrado_bonito(async_client):
    class S:
        async def reponer_del_draft(self, *args, **kwargs): ...
    async def raise_err(*_, **__):
        raise ValueError("jugador_no_encontrado")
    ...
    resp = await async_client.put("/partida/1/reponer_draft?carta_id=1", json={"jugador_id": 9})
    assert resp.status_code == 404
...
@pytest.mark.asyncio
async def test_reponer_draft_jugador_no_en_partida_bonito(async_client):
    class S:
        async def reponer_del_draft(self, *args, **kwargs): ...
    async def raise_err(*_, **__):
