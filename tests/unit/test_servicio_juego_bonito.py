...
async def test_revelar_secreto_fin_por_desgracia_social():
    ...
    repo_j = crear_repo_jugador_mock(obtener_return=jugador)
    repo_s = crear_repo_secreto_mock(obtener_secretos_return=lista_secretos, obtener_secreto_return=secreto)

    servicio = ServicioJuego(partidas=repo_p, jugadores=repo_j, secretos=repo_s)
    # Forzamos fin global a través del método del servicio
    servicio.verificar_fin_por_desgracia_social = AsyncMock(return_value=True)
    with pytest.raises(FinPorDesgraciaSocial):
        await servicio.revelar_secreto(50, 7, 3)
    servicio.verificar_fin_por_desgracia_social.assert_awaited_once_with(50)
...
async def test_revelar_secreto_entrando_en_desgracia_sin_fin_global():
    ...
    servicio = ServicioJuego(partidas=repo_p, jugadores=repo_j, secretos=repo_s)
    # El fin global es falso
    servicio.verificar_fin_por_desgracia_social = AsyncMock(return_value=False)
    with pytest.raises(JugadorEnDesgraciaSocial):
        await servicio.revelar_secreto(51, 8, 5)
    servicio.verificar_fin_por_desgracia_social.assert_awaited_once_with(51)
...
async def test_revelar_secreto_no_llama_fin_global_si_no_entra_en_desgracia():
    ...
    repo_j = crear_repo_jugador_mock(obtener_return=jugador)
    repo_s = crear_repo_secreto_mock(obtener_secretos_return=lista_secretos, obtener_secreto_return=sec_obj)

    servicio = ServicioJuego(partidas=repo_p, jugadores=repo_j, secretos=repo_s)
    # Revelar sec_obj no deja todos revelados (sec_otro sigue oculto) -> no debe entrar en desgracia social
    # Stubear verificación global para comprobar que NO se llama
    servicio.verificar_fin_por_desgracia_social = AsyncMock(return_value=False)
    res = await servicio.revelar_secreto(80, 9, 5)
    assert isinstance(res, RevelarSecretoResultado)
    servicio.verificar_fin_por_desgracia_social.assert_not_awaited()